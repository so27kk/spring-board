...
<script>
    ...
    async function 로그인(){
        ...
        // .value.trim() = 그 중에서 value 로 들어있는 클라이언트가 작성한 이메일 데이터를 양쪽ㅇ 공백 제거하고 const email 공간에 담아두겠다
        ...
        if (!email || !password){
            ...
            return; // 로그인을 할 권한이 없기 떄문에 돌려보내기 아래 코드 실행 못하게 하기
            // return 을 안쓰면 이메일과 비밀번호를 입력하세요. 보여주고 로그인을 시켜주는게 된다
        }
        ...
        const data = await res.json(); // 만약 백엔드에서 응답이 오면 문자열로 온 응답을 다시 json 형태로 변환해서 data 공간 임시보관
