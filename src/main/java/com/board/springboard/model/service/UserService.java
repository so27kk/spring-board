...
public class UserService {
    ...
    public boolean 이메일중복체크기능(String email){
        ...
        이리할 경우에는 개발자가 컨트롤러에 어떻게 데이터를 전달할 것인가?
    ...
    public boolean 회원가입(User user){
        ...
        // 18번쨰 줄에서 개발자가 만들어놓은 이메일 존재 유무 확인하는 기능을 사용해서
        // 만약에 소비자가 작성한 이메일이 존재하는게 사실이라면
        if(이메일중복체크기능(user.getEmail())){
            return false; // 회원가입을 여기서 중단하겠어요 회원가입 못하겠네요 반환
        }
     ...
     */
    // TODO 4. 로그인 메서드를 완성하시오
    // 조건 : 반환타입 = User / 매개변수 = String email
    public User 로그인(String email) {
        // TODO 5. userMapper 의 로그인 메서드를 호출하여 return 하시오
