...
// 만약 @Configuration 이아니라 application.yml 에 쓴다면
// 키이름은 SecurityConfig : 한 줄로 모든 세팅을 다해야하는데 한줄로는 세팅하는데 한계가 있을 떄 사용하는 방법

@Configuration // 설정 파일 세팅 = application.yml 비슷한 효과 application 처럼 한 줄로 설정이 되지 않아
// 코드를 길 게 작성하여 프로젝트 환경 설정을 해야할 떄 작성하는 어노테이션
@EnableWebSecurity // Spring Security 기능을 활성화 밑에 작성한 설정을 진짜로 사용하게끔 확성화 해줘
// 회사와 개발자가 보안설정을 다수 할 수 있다. 어느 순간에는 잠시
...
public class SecurityConfig {
    ...
    public SecurityFilterChain 보안설정(HttpSecurity http) throws Exception{
        ...
        // 위에서 작성한 설정 내용을 최종적으로 완성한 몯습을 반환하겠다.
        return http.build();
    }
    // 클라이언트가 작성한 비밀번호를 스프링에서 만든 비밀번호 암호화 피일을 이용해서 읽을 수 없는 암호화
    @Bean
